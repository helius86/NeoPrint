...
class TestWindow(QWidget):
    ...
    def plot_serial(self):
        current_data = (str(self.esp_serial.readline()))
        print(current_data)
    def connect_ESP32(self):
        self.esp_serial = serial.Serial('COM4', 115200)
        #current_data = (str(serESP32.readline()))
        ## print(current_data)
        #if current_data.startswith("b'"):
        #    activatedForce = current_data.split(';')[1]  # records
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        if mode_select_dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            action_connect_esp32 = QAction('Connect to ESP32', self)
            action_connect_esp32.triggered.connect(self.connect_esp32)
            action_test = QAction('test', self)
            action_test.triggered.connect(self.test)

            operation_menu.addAction(action_homing)
            operation_menu.addAction(action_connect_esp32)
            operation_menu.addAction(action_test)
    ...
    def test(self):
        self.test_window.plot_serial()
    def connect_esp32(self):
        self.test_window.connect_ESP32()
    def homing(self):
