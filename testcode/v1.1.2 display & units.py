...
class TestWindow(QWidget):
    ...
    test_data = ''
    test_done = pyqtSignal()  # Define a signal
                ...
                if response:
                    ...
                    if self.start_resetting:
                        ...
                        if activated_signal == "1":
                            ...
                            #self.test_done_dialog()  # Show the dialog box after the test is completed
...
class SinglePointTestResultWindow(QDialog):
    def __init__(self, parent=None):
        ...
        distance_layout.addWidget(self.distance_unit_label, Qt.AlignLeft)  # Align unit label to the left
        layout.addLayout(distance_layout)

        layout.addWidget(QLabel('Activation force:'))
        force_layout = QHBoxLayout()
        force_layout.addWidget(self.force_label)
        force_layout.addWidget(self.force_unit_label, Qt.AlignLeft)  # Align unit label to the left
...
if __name__ == '__main__':
    ...
    window = MainWindow()
    window.show()
    if(TestWindow.test_done):
        TestWindow.test_done_dialog.show()
    sys.exit(app.exec())
